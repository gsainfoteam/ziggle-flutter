            ...
            child: _BoardAnimation(
              openHidden: () async {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('current using ${channel.ziggleBaseUrl}'),
...
class _BoardAnimationLayoutState extends State<_BoardAnimationLayout> {
    ...
    setState(() {
      ...
      while (_rects.isEmpty || _rects.last.left > 0) {
        ...
        while (rects.isEmpty || rects.last.bottom < maxHeight) {
          ...
          rects.add(Rect.fromLTRB(left, lastBottom + _kGap, right, bottom));
        }
        rects.removeLast();
        _rects.addAll(rects);
      }
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: _rects
          .map((rect) => _Article(rect: rect, onTap: _checkHidden))
          .toList(),
    );
  }

  void _checkHidden(double opacity) {
    _clicks.add(opacity);
    final zip = IterableZip([
      _clicks.reversed.take(2),
      _clicks.reversed.skip(1).take(2),
    ]);
    final opacityCondition = zip.every(
      (element) => element.first - element.last > 0.1,
    );
